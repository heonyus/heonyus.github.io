...
**정의**: 문서 (d)와 단어 (w)의 결합을 **잠재 토픽 (z)** 로 매개하는 확률 모델. 각 문서는 토픽의 혼합비 (,p(z\mid d),)를 갖고, 단어는 (,p(w\mid z),)로 생성.
...
* **모형**
  ...
  * 토픽의 단어분포: (\phi_z=p(w\mid z))

* **추정(EM 개요)**

  * **E-step**: (p(z\mid d,w)\propto p(w\mid z),p(z\mid d))
  * **M-step**:
    (\displaystyle p(w\mid z)\propto \sum_d n(d,w),p(z\mid d,w)),
    (\displaystyle p(z\mid d)\propto \sum_w n(d,w),p(z\mid d,w))
...
* **핵심 한계**:
  ...
  * 실무에서 “fold-in”으로 새 문서에 (\mathbf{\pi}*{d*{\text{new}}})를 **재추정**하지만 이건 **휴리스틱**

> 이 한계를 해결한 게 **LDA**: (\theta_d\sim\text{Dir}(\alpha))라는 **사전**을 두어 문서별 토픽 혼합을 **확률 변수**로 모델링 → **새 문서에도 자연스럽게 확률 할당** 가능, 파라미터 수 (O(KV+K))로 안정.
- **TF-IDF** : 문서를 단어의 빈도 벡터로 변환  
- **Latent Semantic Indexing (LSI)** : 특이값 분해(SVD)를 통해 차원 축소  
- **Probabilistic LSI (pLSI)** : 단어를 *잠재 주제(topic)* 의 혼합으로 표현  

하지만 이러한 방법들은 다음 한계를 가진다.
...
이에 Blei, Ng, Jordan(2003)은 **Latent Dirichlet Allocation (LDA)** 를 제안하였다.  
LDA는 각 문서를 **여러 주제의 확률적 혼합**으로 표현하는  
**3계층 계층적 베이즈 모델(Hierarchical Bayesian Model)** 이다.
...
LDA는 각 문서가 $K$개의 잠재 주제(topic)로 이루어져 있다고 가정한다.  
