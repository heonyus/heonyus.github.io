...
- positive/negative activation set
  - $A_{pos} = \{f_l(x): x \in P_C\}$
  - $A_{neg} = \{f_l(x): x \in N\}$

## Step 3: Concept Activation Vector Training

- $A_{pos}$ vs $A_{neg}$를 input으로 하는 binary linear classifier 학습
  - logistic regression, linear SVM, etc.
- trained classifier의 hyperplane
...
- 이때 weight vector $w$를 **CAV** $v_C^l$로 사용
  - 보통 nomalization(정규화)해서 방향 vector에 집중

> 결과적으로 layer `l`의 activation space에서 concept `C`가 향하는 대표 방향 vector(CAV) $v_C^l$ 를 얻습니다.

## Step 4: Conceptual sensitivity

이제 실제 model prediction에 concept `C`가 얼마나 영향을 주는지 측정합니다.

- 분석 대상 class `k` 설정(ex. "zebra")
- 해당 class로 labeling된 data set $X_k$ 준비
- 각 $x \in X_k$에 대해서 
  - layer `l` activation 계산
    - $z = f_l(x)$
  - class `k` logit 계산
    - $h_{l,k}(z)$
  - gradient 계산
...
$$
S_{C,k,l}(x) = g \cdot v_C^l
$$

> 이 값의 부호(sign)는 concept `C`를 더 강화하면 class `k`의 logit이 증가(+)인지 감소(-)인지를 의미합니다.

## Step 5: TCAV Score Calculation

<img src="https://i.imgur.com/LeUJvfM.png" alt="Figure 1" width="600" referrerpolicy="no-referrer">

아까 말했듯 per-sample sensitivity를 class 전체로 aggregate(부호가 양수인 비율 계산)한 것이 바로 **TCAV score**입니다.
즉 TCAV score는 class `k` 전체에서 concept `C`가 얼마나 중요한지를 평가하는 metric입니다.

$$
\text{TCAV}_{C,k,l} = \frac{\lvert\{x \in X_k : S_{C,k,l}(x) > 0\}\rvert}{\lvert X_k\rvert}
$$

- $\text{TCAV}_{C,k,l} ≈ 0.9$
  - class `k`의 90% sample에 대해 concept `C`를 강화하면 logit이 증가하는 것
  - concept `C`가 class `k` prediction에 강하게 기여
- $\text{TCAV}_{C,k,l} ≈ 0.5$
  - 거즘 random 수준 -> 뚜렷한 영향 없음
- $\text{TCAV}_{C,k,l} < 0.5$
  - 오히려 concept `C`가 class `k` prediction을 방해하는 것

# Conclusion

TCAV는 다음과 같은 특징을 가진 **concept-based post-hoc XAI** 기법입니다.

1. 사람이 정의한 concept C(예시 이미지 집합)에서
   layer activation space의 **CAV 방향**을 추출
2. 그 방향에 대한 **directional derivative**를 이용해 개별 샘플에서의 concept sensitivity를 계산
3. class 전체에 대해 “C를 강화하면 logit이 증가하는 샘플 비율”을
   **TCAV score(0~1)** 로 정의
4. “이 모델이 class k를 예측할 때 concept C에 얼마나 의존하는가”를 글로벌하게 수치화